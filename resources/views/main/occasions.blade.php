    ...
    <div class="container-fluid page-header mb-5 p-0" style="background-image: url('{{ (!empty($page_property_view->slider_images[1]->image)) ? asset('page_assets/img/' . $page_property_view->slider_images[1]->image) : asset('upload/No_Image_Available.jpg') }}');">
        <div class="container-fluid page-header-inner py-5">
            <div class="container text-center">
                <h1 class="display-3 text-white mb-3 animated slideInDown">Occasions</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center text-uppercase">
                        <li class="breadcrumb-item"><a href="{{ route('index') }}">Home</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('occasions') }}">Occasions</a></li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <section id="hotels" class="section-with-bg">
      ...
      <div class="container" data-aos="fade-up">
        ...
        <div class="row" data-aos="fade-up" data-aos-delay="100">
            ...
            @if( $occasions && count($occasions) > 0 )
                @foreach($occasions as $key => $occasion)
                    ...
                    <div class="col-lg-4 col-md-6 ">
                        <div class="hotel" style="width: 300px; max-height: 550px; min-height: 550px; object-fit: cover;">
                            <div class="hotel-img">
                            <img src="{{ (!empty($occasion->image1)) ? url('upload/occasion_images/'.$occasion->image1) : url('upload/No_Image_Available.jpg') }}" style="width: 300px; max-height: 300px; min-height: 300px; object-fit: cover;" alt="Hotel 1" class="img-fluid">
                            </div>
                            ...
                            <a href="https://api.whatsapp.com/send?phone={{ $page_property_view->whatsapp_number }}&text=I%20want%20to%20know%20about%20this%20product.%0AProduct%20Title%3A%20{{ urlencode($occasion->title) }}%0APrice%3A%20%E2%82%B9{{ ($occasion->price > 0) ? (($occasion->offer_percent > 0) ? (number_format($occasion->price - ($occasion->price * ($occasion->offer_percent / 100)), 2)) : (number_format($occasion->price, 2))) : '' }}%0AProduct%20url%3A%20{{ urlencode(route('single.occasion', $occasion->id)) }}" target="_blank" class="btn btn-success rounded-pill py-2 px-4 d-flex align-items-center justify-content-center mb-3"
